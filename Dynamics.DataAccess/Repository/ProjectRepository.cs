    ...
    public class ProjectRepository : IProjectRepository
    {
        ...
        public async Task<bool> ShutdownProjectAsync(ShutdownProjectVM entity)
        {
            ...
            if (projectObj != null)
            {
                ...
                projectObj.Reason = entity.Reason;
        ...
        public async Task<List<string>> GetStatisticOfProject(Guid projectID)
        {
            ...
            if (projectObj != null)
            {
                var projectResouceMoney = _db.ProjectResources.FirstOrDefaultAsync(x => x.ProjectID.Equals(projectID) && x.ResourceName.Equals("Money") && x.Unit.Equals("VND")).Result;
                if (projectResouceMoney != null)
                {
                    var progressValue = (double)projectResouceMoney.Quantity / projectResouceMoney.ExpectedQuantity * 100;
                    //cal nums contributor
                    var numberOfProjectContributor = _db.UserToProjectTransactionHistories
                        .Where(x => x.ProjectID.Equals(projectID) && x.Status==1)
                        .Select(x => x.UserID)
                        .Distinct().Count();                  
                    numberOfProjectContributor += _db.OrganizationToProjectTransactionHistory
                        ...
                        .Distinct().Count();
                    var timeLeft = projectObj.EndTime.HasValue ? projectObj.EndTime.Value.ToDateTime(TimeOnly.MinValue) - DateTime.Now : TimeSpan.Zero;
                    List<string> statistic = new List<string>()
            {
                projectResouceMoney.Quantity.ToString(),projectResouceMoney.ExpectedQuantity.ToString(),progressValue.ToString(),numberOfProjectContributor.ToString(),timeLeft.ToString()
            };
                    return statistic;
                }
            }           
        ...
        public async Task<List<Project>> GetAllActiveProjectsAsync(string? includeObjects = null)
        {
        ...
        {
            IQueryable<ProjectMember> leaderProjectMembers = _db.ProjectMembers.Where(x=>x.ProjectID.Equals(projectID) && x.Status==2);
        ...
        public async Task<bool> UpdateAsync(Project entity, Guid newProjectLeaderID)
        {
            ...
            existingItem.Address = entity.Address;
            ...
            var oldProjectLeader = await _db.ProjectMembers.FirstOrDefaultAsync(x => x.Status == 2 && x.ProjectID.Equals(entity.ProjectID));
           var newProjectLeader = await _db.ProjectMembers.FirstOrDefaultAsync(x=>x.UserID.Equals(newProjectLeaderID) && x.ProjectID.Equals(entity.ProjectID));
            if (oldProjectLeader != null && newProjectLeader!=null && !oldProjectLeader.UserID.Equals(newProjectLeader.UserID))
            {
        ...
        public List<ProjectMember> FilterProjectMember(Expression<Func<ProjectMember, bool>> filter, string? includeObjects = null)
        {
        ...
        public async Task<List<User>> GetAllMemberOfProjectIDAsync(Guid projectID)
        {
            ...
            if (project != null)
            {
                project.ProjectMember = _db.ProjectMembers.Where(x => x.ProjectID.Equals(projectID) && x.Status==1).Include("User").ToList();
        ...
        {
            var memberObj = _db.ProjectMembers.FirstOrDefault(x => x.UserID.Equals(memberID) && x.ProjectID.Equals(projectID));
            try
            {
                if (memberObj != null)
                {
        ...
        {
            var res = await _db.ProjectMembers.AddAsync(new ProjectMember { UserID = memberID, ProjectID = projectID, Status = 0 });
            await _db.SaveChangesAsync();
            return res!=null;
        }
        public async Task<bool> AcceptedJoinRequestAsync(Guid memberID,Guid projectID)
        {
            var memberObj = await _db.ProjectMembers.FirstOrDefaultAsync(x => x.UserID.Equals(memberID) && x.ProjectID.Equals(projectID));
            if (memberObj != null)
            {
                memberObj.Status = 1;
       ...
       public async Task<bool> DenyJoinRequestAsync(Guid memberID, Guid projectID)
        ...
        public async Task<bool> HandleResourceAutomatic(Guid transactionID, string donor)
        {
            ...
            {
               var transactionObj =  await _db.UserToProjectTransactionHistories.Where(x => x.TransactionID.Equals(transactionID)).Include("Resource").FirstOrDefaultAsync();
                
                if (transactionObj != null)
                {//check if the resource name of transaction is in the list of resource type, if yes, update the quantity of resource
                    if (transactionObj.ResourceID.HasValue && resourceTypes.Contains(transactionObj.ResourceID.Value))
                    {
                        //find the obj resource in ProjectResource table
                        var resourceObj = await _db.ProjectResources.FirstOrDefaultAsync(x => x.ResourceID.Equals(transactionObj.ResourceID));
                        if (resourceObj != null)
                        {
            ...
            {
                 var transactionObj = await _db.OrganizationToProjectTransactionHistory.FirstOrDefaultAsync(x => x.TransactionID.Equals(transactionID));
                if (transactionObj != null)
                {//check if the resource name of transaction is in the list of resource type, if yes, update the quantity of resource
                    if (transactionObj.ResourceID.HasValue && resourceTypes.Contains(transactionObj.ResourceID.Value))
                    {
        ...
        public async Task<IEnumerable<ProjectResource?>> FilterProjectResourceAsync(Expression<Func<ProjectResource, bool>> filter)
        {
       ...
       public async Task<bool> AddResourceTypeAsync(ProjectResource entity)
        ...
        public async Task<bool> DeleteResourceTypeAsync(Guid resourceID)
        {
            ...
            var existUserDonate = await _db.UserToProjectTransactionHistories.FirstOrDefaultAsync(x => x.ResourceID.Equals(resourceID));
            var existOrgDonate = await _db.OrganizationToProjectTransactionHistory.FirstOrDefaultAsync(x => x.ResourceID.Equals(resourceID));
        ...
        public async Task<List<UserToProjectTransactionHistory>> GetAllUserDonateAsync(Expression<Func<UserToProjectTransactionHistory, bool>> filter, string? includeObjects = null)
        {
            IQueryable<UserToProjectTransactionHistory> listUserDonate = _db.UserToProjectTransactionHistories.Where(filter);
            if (!string.IsNullOrEmpty(includeObjects))
            {
                foreach (var includeObj in includeObjects.Split(
               new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
        ...
        public async Task<List<OrganizationToProjectHistory>> GetAllOrganizationDonateAsync(Expression<Func<OrganizationToProjectHistory, bool>> filter, string? includeObjects = null)
        {
            IQueryable<OrganizationToProjectHistory> listOrganizationDonate = _db.OrganizationToProjectTransactionHistory.Where(filter);
            if (!string.IsNullOrEmpty(includeObjects))
            {
                foreach (var includeObj in includeObjects.Split(
               new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
        ...
        {
            var transactionObj = await _db.UserToProjectTransactionHistories.FirstOrDefaultAsync(x => x.TransactionID.Equals(transactionID));
            if(transactionObj != null)
            {
       ...
       public async Task<bool> AcceptedOrgDonateRequestAsync(Guid transactionID)
        ...
        public async Task<List<History>?> GetAllPhaseReportsAsync(Expression<Func<History, bool>> filter, string? includeObjects = null)
        {
       ...
       public async Task<bool> EditPhaseReportAsync(History entity)
        ...
        {
            var history = await _db.Histories.FirstOrDefaultAsync(x => x.ProjectID.Equals(projectID) && x.HistoryID.Equals(historyID));
            if(history != null)
            {
