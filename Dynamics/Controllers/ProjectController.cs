...
namespace Dynamics.Controllers
{
    public class ProjectController : Controller
    {
        ...
        {
            //get all project
            var allProjects = await projectRepository.GetAllActiveProjectsAsync("Organization,Request");
            //get project that user has joined
            var projectMemberList = projectRepository.FilterProjectMember(x => x.UserID.Equals(userID) && x.Status >= 1 && x.Project.ProjectStatus >=1, "Project");
            ...
            {
                var project = await projectRepository.GetProjectAsync(x => x.ProjectID.Equals(projectMember.ProjectID), "Organization,Request");
                if (project != null)
                {
            ...
            return View(new MyProjectVM() { ProjectsILead = projectsILead, ProjectsIAmMember = projectsIAmMember, OtherProjects = otherProjects });
        }
        ...
        public async Task<IActionResult> DeleteImage(string imgPath, Guid phaseID)
        {
            ...
            if (phaseID!=Guid.Empty)
            {
                ...
                var historyObj = await projectRepository.GetAllPhaseReportsAsync(x => x.HistoryID.Equals(phaseID), "Project");
                if (historyObj != null&&allImagesOfPhase!=null)
                {
                    ...
                    if (!res)
                    {
                        ...
                        return RedirectToAction(nameof(EditProjectPhaseReport), new { historyID = phaseID, projectID = currentProjectID });
                    }
            ...
            {
                var allImagesOfProject = await projectRepository.GetAllImagesAsync(new Guid(currentProjectID), "Project");
                var projectObj = await projectRepository.GetProjectAsync(x => x.ProjectID.Equals(new Guid(currentProjectID)), "Request,Organization");               
        ...
        public async Task<IActionResult> ImportFileProject(FinishProjectVM finishProjectVM, IFormFile reportFile)
        {
            ...
            var projectObj = await projectRepository.GetProjectAsync(p => p.ProjectID.Equals(projectID), "Organization,Request");
            if (projectObj != null)
            {
                if (reportFile != null)
                {
            ...
            if (!string.IsNullOrEmpty(fileWebPath))
            {
                ...
                if (!string.IsNullOrEmpty(fileExtension))
                {
                    switch (fileExtension)
                    {
                        ...
                        case ".docx":
                            return File(System.IO.File.ReadAllBytes(filepath), "application/vnd.openxmlformats-officedocument.wordprocessingml.document", System.IO.Path.GetFileName(filepath));
                        case ".doc":
                            return File(System.IO.File.ReadAllBytes(filepath), "application/msword", System.IO.Path.GetFileName(filepath));
                        case ".txt":
        ...
        {
            var projectObj = await projectRepository.GetProjectAsync(p => p.ProjectID.Equals(id), "Organization,Request");
            if (projectObj == null)
            {
                return NotFound();
            }
            ...
            List<User> memberList = projectRepository.FilterMemberOfProject(x => x.ProjectID.Equals(id) && x.Status >= 1);
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateProjectProfile(UpdateProjectProfileRequestDto updateProject, List<IFormFile> images)
        {
            var existingObj = await projectRepository.GetProjectAsync(p => p.ProjectID.Equals(updateProject.ProjectID), "Organization,Request");
        ...
        public async Task<IActionResult> ShutdownProject(ShutdownProjectVM shutdownProjectVM)
        {
            ...
            if (res)
            {
                TempData[MyConstants.Success] = "Shutdown project successfully!";
                TempData[MyConstants.Info] = "You just have shut down a project for " + shutdownProjectVM.Reason;
                return RedirectToAction(nameof(Index), new { id = new Guid(userIDString) });
            }
            TempData[MyConstants.Error] = "Fail to shut down project!";
            return RedirectToAction(nameof(UpdateProjectProfile), new { id = shutdownProjectVM.ProjectID});
        }
        ...
        public async Task<IActionResult> ManageProject(string id)
        {
            ...
            var projectObj = await projectRepository.GetProjectAsync(p => p.ProjectID.Equals(new Guid(id)), "Organization,Request");
            if (projectObj != null)
            {
                projectObj.Request = await requestRepository.GetRequestAsync(r => r.RequestID.Equals(projectObj.RequestID), "User");
        ...
        {
            var allProjectMember = projectRepository.FilterProjectMember(p => p.ProjectID.Equals(projectID) && p.Status >= 1, "User");

            if (allProjectMember == null)
            {
                return NotFound();
            }
            var joinRequests = projectRepository.FilterProjectMember(p => p.ProjectID.Equals(projectID) && p.Status == 0, "User") ?? Enumerable.Empty<ProjectMember>();
        ...
        public async Task<IActionResult> JoinProjectRequest(Guid memberID, Guid projectID)
        {
            ...
            var existingJoinRequest = projectRepository.FilterProjectMember(p => p.ProjectID.Equals(projectID) && p.UserID.Equals(memberID) && p.Status == 0, "User").FirstOrDefault();
        ...
        {
            var allJoinRequest = projectRepository.FilterProjectMember(p => p.ProjectID.Equals(projectID) && p.Status == 0, "User");
        ...
        {
            var currentProjectID = HttpContext.Session.GetString("currentProjectID");
            var allJoinRequest = projectRepository.FilterProjectMember(p => p.ProjectID.Equals(new Guid(currentProjectID)) && p.Status == 0, "User");
            if (allJoinRequest == null)
            {
                return NotFound();
            }
            ...
            {
                var res = await projectRepository.AcceptedJoinRequestAsync(joinRequest.UserID, new Guid(currentProjectID));
                if (!res)
                {
                    TempData[MyConstants.Error] = "Failed to accept the join request!";
        ...
        public async Task<IActionResult>SendDonateRequest(Guid projectID,string donor)
        {
            ...
            var allResource = await projectRepository.FilterProjectResourceAsync(p => p.ProjectID.Equals(projectID) && !p.ResourceName.Equals("Money"));
            if (allResource == null)
            {
                return NotFound();
            }
            ...
            if (!string.IsNullOrEmpty(donor)&&donor.Equals("User"))
            {
                ...
                if (currentUserID != null)
                {
                    donateHistoryOfUser = await projectRepository.GetAllUserDonateAsync(u => u.UserID.Equals(new Guid(currentUserID)) && u.ProjectID.Equals(projectID), "User,Resource");
            ...
            else if (!string.IsNullOrEmpty(donor) && donor.Equals("Organization"))
            {
                ...
                if (currentUserID != null)
                {
                    var OrganizationOfUser = await organizationRepository.GetOrganizationOfAUser(new Guid(currentUserID));
                    var OrganizationOfUserID = OrganizationOfUser.OrganizationID;
                    ViewData["OrganizationOfUserID"] = OrganizationOfUserID;
                    donateHistoryOfOrg = await projectRepository.GetAllOrganizationDonateAsync(u => u.OrganizationID.Equals(OrganizationOfUserID) && u.ProjectID.Equals(projectID), "Organization,Resource");
                    sendDonateRequestVM = new SendDonateRequestVM()
                    {
                        ProjectID = projectID,
                        TypeDonor = donor,
        ...
        public async Task<IActionResult> SendDonateRequest(SendDonateRequestVM sendDonateRequestVM)
        {
            ...
            {
                if (!string.IsNullOrEmpty(sendDonateRequestVM.TypeDonor) && sendDonateRequestVM.TypeDonor.Equals("User"))
                {
                    var currentUserID = HttpContext.Session.GetString("currentUserID");
                    if (currentUserID != null)
                    {//write code
                        ...
                        if (!res)
                        {
                            TempData[MyConstants.Error] = "Fail to send user donate request!";
                            return RedirectToAction(nameof(SendDonateRequest), new { projectID = sendDonateRequestVM.ProjectID, donor = "User" });
                        }
                    }
                }
                else if (!string.IsNullOrEmpty(sendDonateRequestVM.TypeDonor) && sendDonateRequestVM.TypeDonor.Equals("Organization"))
                {

                        var res = await projectRepository.AddOrgDonateRequestAsync(sendDonateRequestVM.OrgDonate);
                        if (!res)
                        {
                            TempData[MyConstants.Error] = "Fail to send organization donate request!";
                            return RedirectToAction(nameof(SendDonateRequest), new { projectID = sendDonateRequestVM.ProjectID, donor = "Organization" });
                        }
                    
                }
                TempData[MyConstants.Success] = "Send donate request successfully!";
                return RedirectToAction(nameof(SendDonateRequest), new { projectID = sendDonateRequestVM.ProjectID, donor = "User" });
            }
            TempData[MyConstants.Error] = "Fail to send donate request!";
            return RedirectToAction(nameof(SendDonateRequest), new {projectID= sendDonateRequestVM.ProjectID, donor="User"});
        ...
        {
            var allUserDonate = await projectRepository.GetAllUserDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 1, "User,Resource");
            if (allUserDonate == null)
            {
                return NotFound();
            }

            var allOrganizationDonate = await projectRepository.GetAllOrganizationDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 1, "Organization,Resource");
            ...
            int nums = (await projectRepository.GetAllUserDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 0, "User,Resource") ?? new List<UserToProjectTransactionHistory>()).Count();
            var hasUserDonateRequest = nums > 0;

            int nums2 = (await projectRepository.GetAllOrganizationDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 0, "Organization,Resource") ?? new List<OrganizationToProjectHistory>()).Count();
        ...
        {
            var allUserDonate = await projectRepository.GetAllUserDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 0, "User,Resource");
            if (allUserDonate == null)
            {
                return NotFound();
            }
        ...
        {
            var allOrgDonate = await projectRepository.GetAllOrganizationDonateAsync(u => u.ProjectID.Equals(projectID) && u.Status == 0, "Organization,Resource");
        ...
        public async Task<IActionResult> AcceptedDonateRequest(Guid transactionID, string donor)
        {
            ...
            if (res)
            {
                ...
                return (donor.Equals("User")) ? RedirectToAction(nameof(ReviewUserDonateRequest), new { id = HttpContext.Session.GetString("currentProjectID") }) :
                     RedirectToAction(nameof(ReviewOrgDonateRequest), new { id = HttpContext.Session.GetString("currentProjectID") });
            }
        ...
        public async Task<IActionResult> AcceptedDonateRequestAll(string donor)
        {
            ...
            {
                case "User":
                    var allUserDonateRequest = await projectRepository.GetAllUserDonateAsync(u => u.ProjectID.Equals(new Guid(currentProjectID)) && u.Status == 0, "User");
                    ...
                    foreach (var userDonate in allUserDonateRequest)
                    {
                        ...
                        if (!res)
                        {
                            ...
                            return RedirectToAction(nameof(ReviewUserDonateRequest), new { id = HttpContext.Session.GetString("currentProjectID") });
                        }
                    }
                    break;
                case "Organization":
                    var allOrgDonateRequest = await projectRepository.GetAllOrganizationDonateAsync(u => u.ProjectID.Equals(new Guid(currentProjectID)) && u.Status == 0, "Organization");
                    ...
                    foreach (var orgDonate in allOrgDonateRequest)
                    {
                        ...
                        {
                            TempData[MyConstants.Error] = "Failed to accept all the donation request!";
                            return RedirectToAction(nameof(ReviewOrgDonateRequest), new { id = HttpContext.Session.GetString("currentProjectID") });
                        }
                    }
                    break;
            }
        ...
        {
            IEnumerable<SelectListItem> ResourceTypeList = projectRepository.GetAllProjectResourceQuery().Select(x => new SelectListItem
            {
                Text = x.ResourceName,
                Value = x.ResourceID.ToString(),
            });
        ...
        {           
            if (ModelState.IsValid)
            {
                var projectObj = await projectRepository.GetProjectAsync(p => p.ProjectID.Equals(projectResource.ProjectID), "Organization,Request");
                if (projectObj != null)
                {
            ...
            return RedirectToAction(nameof(ManageProjectResource), new { id = HttpContext.Session.GetString("currentProjectID") });
        }

        [HttpPost]
        public async Task<IActionResult> UpdateResourceType(ProjectResource projectResource)
        {
            var projectResourceObj = await projectRepository.FilterProjectResourceAsync(p => p.ResourceID.Equals(projectResource.ResourceID));
        ...
        {
            var allUpdate = await projectRepository.GetAllPhaseReportsAsync(u => u.ProjectID.Equals(projectID), "Project");
            if (allUpdate == null)
            {
                return NotFound();
            }
        ...
        {
            var projectUpdates = await projectRepository.GetAllPhaseReportsAsync(u => u.HistoryID.Equals(historyID), "Project");
            ...
            var existingDates = await projectRepository.GetExistingReportDatesAsync(projectID);
            // Convert to a list of string dates in "YYYY-MM-DD" format
            var disabledDates = existingDates.Select(d => d.ToString("yyyy-MM-dd")).Except(new[] { projectUpdate?.Date.ToString("yyyy-MM-dd") }).ToList();
            ViewBag.DisabledDates = disabledDates;
        ...
        public async Task<IActionResult> DeleteProjectPhaseReport(Guid id)
        {
            ...
            if (res)
            {
                ...
                return RedirectToAction(nameof(ManageProjectPhaseReport), new { id = new Guid(HttpContext.Session.GetString("currentProjectID")) });
            }
